...
func (p *Client) processMsg(msg proto.Message) (err error) {
	...
	case proto.UserSendMessageCmd:
        ...
        case proto.UserSendMessageP2PCmd:
                err = p.proccessUserSendMessageP2P(msg)
	default:
...
func (p *Client) SendMessageToUser(userId int, text string) {
	...
	if err != nil {
        ...
        fmt.Println("SendMessageToUser: ", string(data))
...
func (p *Client) SendMessageToUserP2P(userId int, text string) {

        var respMsg proto.Message
        respMsg.Cmd = proto.UserRecvMessageP2PCmd

        var recvMsg proto.UserRecvMessageP2PReq
        recvMsg.UserId = userId
        recvMsg.Data = text
...
func (p *Client) proccessUserSendMessageP2P(msg proto.Message) (err error) {
        var userReq proto.UserSendMessageP2PReq
        err = json.Unmarshal([]byte(msg.Data), &userReq)
        if err != nil {
                fmt.Println("unmarshal failed, err:", err)
                return
        ...
        users := clientMgr.GetAllUsers()
        client, ok := users[userReq.To]
        if !ok {
            fmt.Println("find to failed.")
            return
        }
        client.SendMessageToUserP2P(userReq.From, userReq.Data)
        return
}
